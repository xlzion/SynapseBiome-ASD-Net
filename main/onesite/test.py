...
def main():
    ...
    for site in sites:
        ...
        try:
            # 创建留一站点验证加载器
            fmri_loader = LeaveOneSiteOutLoader(file_path=file_path, graph_type="cc200", batch_size=32, site_name=site)
            microbe_loader = MicrobeDataLoader(csv_path=csv_path, biom_path=biom_path, batch_size=32)

            # 获取当前站点的数据加载器
            fmri_loaders = fmri_loader.get_dataloaders(0)  # 使用索引0，因为每个站点只有一个划分
            ...
            best_model.load_state_dict(torch.load(f"/home/yangzongxian/xlz/ASD_GCN/main/onesite/best_model_site_{site}.pth"))
            ...
            print(f"站点 {site} 测试准确率: {test_acc:.2%}")

            # 进行对抗测试
            acc_original, acc_perturbed = test_adversarial(best_model, test_loader, device, criterion)
            print(f"站点 {site} 原始测试准确率: {acc_original:.2%}")
            print(f"站点 {site} 对抗测试准确率: {acc_perturbed:.2%}")
